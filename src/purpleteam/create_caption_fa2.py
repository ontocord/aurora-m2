...
from transformers import CLIPProcessor, CLIPModel, AutoModel, AutoTokenizer, AutoModelWithLMHead, BitsAndBytesConfig
...
def setup(args, d):
  ...
  if fluo_model is None:
    ...
    # fluo_model = AutoModelForCausalLM.from_pretrained(args.caption_generator_model_path, trust_remote_code=True, load_in_4bit=True, attn_implementation="flash_attention_2", device_map={"": id}, cache_dir=args.cache_dir).eval()
    fluo_model = AutoModelForCausalLM.from_pretrained(args.caption_generator_model_path, trust_remote_code=True, torch_dtype=bfloat16, attn_implementation="flash_attention_2", cache_dir=args.cache_dir).to(device).eval()
...
def main():
    ...
    parser.add_argument("--batch_size", type=int, default=150, help="Batch size") # 70
