...
distributed_state = PartialState(num_processes=num_devices)

def setup(args):
  fluo_model = AutoModelForCausalLM.from_pretrained(args.caption_generator_model_path, attn_implementation="flash_attention_2", trust_remote_code=True, torch_dtype=torch.bfloat16, cache_dir=args.cache_dir).eval() # 'auto' not dupported
  ...
  fluo_model = fluo_model.to(distributed_state.device)
  

  return fluo_model, fluo_processor
...
def generate_captions(idx_and_images, suffix: str = "", batch_size: int = 4, score_cutoff: int = 0.2):
    time0 = time.time()
    print (os.getpid(), num_devices, len(idx_and_images), fluo_model.device)
    ...
    with torch.no_grad():
      ...
      inputs = fluo_processor(text=[fluo_prompt]*len(images), images=images, return_tensors="pt").to(distributed_state.device)
    ...
    print (len(images), len(idxs))    
    time1 = time.time()
    print (fluo_model.device, generated_texts)
    print(f"@TIME | 'create_caption_from_img' | Caption Generation: {time1 - time0:.2f} seconds")
...
def main():
    ...
    fluo_model, fluo_processor = setup(args)
    ...
    with open(args.output_path, "w") as outfile: 
      for file in glob.glob(args.input_dir.rstrip("/")+"/*/*/*/*"):
        ...
        for rng in range(0, len(image_array), args.batch_size*num_devices):
          ...
          print (distributed_state.num_processes, num_devices)
          print (accelerator.num_processes)
          collator = []
          with distributed_state.split_between_processes(list(enumerate(images))) as images_split:
