...
echo 'generating generate_fixed_and_upsampled_captions data'

srun python -m src.create_multimodal_data \
      --batch_size 4 \
      --task generate_fixed_and_upsampled_captions \
      --use_LLM_size medium \
      --cache_dir $cache_dir \
      --input_path $directory/commoncatalog_captions_then_images.jsonl \
      --output_dir $directory \
      --output_path $directory/fixed_and_upsampled_images.jsonl  \
