...
def generate_captions(images, suffix: str = "", score_cutoff: int = 0.2):
    ...
    up_prompt = []
    for prompt2, e1, e2 , image_idx in zip(working_prompt2_2, elements1, elements1_1, range(len(images))):
      e1 = e1.strip().replace("  ", " ")
      e2 = e2.strip().replace("  ", " ")
    ...
    with open(args.output_path, "w") as outfile: 
      for file in glob.glob(args.input_dir.rstrip("/")+"/*/*/*/*"):
        ...
        for image, caption, blip_text, title, usertags, url in zip(df['jpg'], df['caption'], df['blip2_caption'], df['title'], df['usertags'], df['downloadurl']):
          all_data.append({'image': image, 'orig_caption': caption, 'blip2_text': blip_text, 'title': title, 'usertags': usertags, 'url': url, 'source': file})
